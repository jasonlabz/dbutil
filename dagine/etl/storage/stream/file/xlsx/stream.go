...
func (s *Stream) Rows(conf *jsonutil.JSON) (file.Rows, error) {
	return NewRows(s.file, conf)
}

// Writer 新建一个配置未conf的xlsx流写入器
func (s *Stream) Writer(conf *jsonutil.JSON) (file.StreamWriter, error) {
...
func NewRows(f *excelize.File, c *jsonutil.JSON) (rows *Rows, err error) {
	var conf *InConfig
...
func NewWriter(f *excelize.File, c *jsonutil.JSON) (file.StreamWriter, error) {
	w := &Writer{
		file:    f,
		columns: make(map[int]Column),
	}
