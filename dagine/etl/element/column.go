...
// ColumnValueCloneable 可克隆列值
type ColumnValueCloneable interface {
	Clone() ColumnValue //克隆
}

// ColumnValueComparable 可比较列值
type ColumnValueComparable interface {
	// Cmp 比较 1代表大于， 0代表相等， -1代表小于
...
func (d *DefaultColumn) Cmp(c Column) (int, error) {
	...
	comparableColumn, ok := d.ColumnValue.(ColumnValueComparable)
	if !ok {
		return 0, ErrNotColumnValueComparable
	}
	return comparableColumn.Cmp(c)
}

// Clone 克隆列，如果不是可克隆列值，就会报错
func (d *DefaultColumn) Clone() (Column, error) {
	cloneable, ok := d.ColumnValue.(ColumnValueCloneable)
	if !ok {
		return nil, ErrNotColumnValueCloneable
	}

	return &DefaultColumn{
		ColumnValue: cloneable.Clone(),
		name:        d.name,
		byteSize:    d.byteSize,
	}, nil
}
